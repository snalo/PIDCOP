...
class Stocastic_MRRVDP(VDP):
    ...
    def __init__(self, ring_radius, pitch, vdp_type, supported_layer_list=[], BR=30) -> None:
        ...
        self.br = BR*1e9  # * Bit rate is defined in BPS which defines the latency of the VDP unit in digital domain
    ...
    def set_vdp_latency(self) -> float:
        ...
        if self.vdp_type == 'AMM':
            ...
            self.mod_latency = (
                2**(self.vdp_element_list[0].precision))/(self.br)
            # self.latency = self.mod_latency+self.pheripheral_latency
            self.latency = self.mod_latency
