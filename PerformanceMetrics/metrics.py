...
class Metrics:
    ...
    def get_dynamic_energy(self, accelerator, utilized_rings):
        ...
        dynamic_energy_comp['mzi_energy'] = 0
        ...
        for vdp in accelerator.vdp_units_list:
            ...
            if accelerator.vdp_type in ('SPOGA', 'PIDCOP'):
                ...
                s_a_energy = 0
            if accelerator.vdp_type == 'PIXEL_OO':
                ...
                mzi_energy = self.mzi.energy
                pd_energy = mzi_energy #placeholder for mzi_energy
            else:
                pd_energy = vdp.calls_count * vdp.get_element_count() * self.pd.energy
                tia_energy = vdp.calls_count * vdp.get_element_count() * self.tia.energy
                s_a_energy = vdp.calls_count * self.adder.energy
    ...
    def get_static_power(self, vdp_type, unit_count, N, M):
        ...
        if vdp_type == 'HOLYLIGHT':
            ...
            laser_power = self.laser_power_per_wavelength*elements_count*element_size  #Check
            power_params = {}
            power_params['adc'] = no_of_adc*self.adc.power
            power_params['dac'] = no_of_dac*self.dac.power
            power_params['pd'] = no_of_pd*self.pd.power
        ...
        if vdp_type == 'SPOGA':
            ...
            no_of_mrr = 12*elements_count*element_size #Changed to 12
            laser_power = self.laser_power_per_wavelength*elements_count*element_size
            power_params = {}
            power_params['adc'] = no_of_adc*self.adc.power
            power_params['dac'] = no_of_dac*self.dac.power
        ...
        if vdp_type == 'PIXEL_OO':
            no_of_adc = elements_count
            no_of_dac = elements_count*element_size*8
            no_of_pd = elements_count
            no_of_tia = elements_count
            no_of_mrr = 8*elements_count*element_size #Changed to 8
            no_of_mzi = 4*elements_count*element_size
            laser_power = self.laser_power_per_wavelength*elements_count*element_size
            power_params = {}
            power_params['adc'] = no_of_adc*self.adc.power
            power_params['dac'] = no_of_dac*self.dac.power
            ...
            power_params['mrr'] = no_of_mrr * \
                (self.mrr.power_eo+self.mrr.power_to)
            power_params['laser_power'] = laser_power
            power_params['mzi'] = no_of_mzi * self.mzi.power
            power_params['laser_power'] = laser_power

            vdp_power += no_of_adc*self.adc.power + no_of_dac*self.dac.power + no_of_pd*self.pd.power + no_of_tia * \
                self.tia.power + no_of_mrr * (self.mrr.power_eo+self.mrr.power_to) + no_of_mzi*self.mzi.power + \
                laser_power*self.wall_plug_efficiency
            # print("VDP Power ", vdp_power)
        pheripheral_power_params = {}
        pheripheral_power_params['io'] = self.io_interface.power
    ...
    def get_total_area(self, TYPE, unit_count, N, M):
        ...
        voltage_adder = 0 #mm2
        tir = 0 #mm2
        mzi_total_area = self.mzi.area #m = (arm_length + 2 * bend_radius) * (spacing_between_arms + 2 * waveguide_width)
        ...
        if TYPE == 'HOLYLIGHT':
            ...
            return total_area
        elif TYPE == 'PIXEL_EO': #Optical Electrical
            print('PIXEL EO (AMW)')

            mrr_area = (3.14 * (radius ** 2) * 1e-6)

            dp_unit_area = (mrr_area * 8 * N * M) 
            ...
            return total_area
        elif TYPE == 'PIXEL_OO': #Optical Optical
            print('PIXEL OO (AMW)')

            mrr_area = (3.14 * (radius ** 2) * 1e-6)

            dp_unit_area = (mzi_total_area * 4 * N * M) + (mrr_area * 8 * N * M)
            ...
            dp_unit_area = mrr_area * (12 * N * M) + (4 * M) + (8 * N) #Explains MRR in OAME + MRRs outside b4 multiplexing + MRRs in Aggregation Lane
